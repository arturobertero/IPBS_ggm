...
create_facet_plot <- function(data) {
  ...
  ggplot(data_wide) +
    ...
    facet_wrap(~ type, scales = "free") +  # Create facets for `educ` and `pint`
    xlab("") +
    ylab("R²") +
...
```{r}
# Function to calculate ASPL, exclude outliers, and return bootstrapped 95% CIs
aspl_from_boots_with_ci <- function(df_boot) {
  ...
  ci_upper <- quantile(aspl_values_filtered, 0.975, na.rm = TRUE)  # 97.5th percentile
...
# Prepare the data for box plot (using the filtered ASPL values without outliers)
plot_data <- data.frame(
  ...
  mean_aspl = c(rep(aspl_pint_l_2_1_pc$mean_aspl, length(aspl_pint_l_2_1_pc$aspl_values)),
                rep(aspl_pint_h_2_1_pc$mean_aspl, length(aspl_pint_h_2_1_pc$aspl_values)),
                rep(aspl_educ_l_pc$mean_aspl, length(aspl_educ_l_pc$aspl_values)),
                rep(aspl_educ_h_pc$mean_aspl, length(aspl_educ_h_pc$aspl_values))),
  ci_lower = c(rep(aspl_pint_l_2_1_pc$ci_lower, length(aspl_pint_l_2_1_pc$aspl_values)),
               rep(aspl_pint_h_2_1_pc$ci_lower, length(aspl_pint_h_2_1_pc$aspl_values)),
               rep(aspl_educ_l_pc$ci_lower, length(aspl_educ_l_pc$aspl_values)),
               rep(aspl_educ_h_pc$ci_lower, length(aspl_educ_h_pc$aspl_values))),
  ci_upper = c(rep(aspl_pint_l_2_1_pc$ci_upper, length(aspl_pint_l_2_1_pc$aspl_values)),
               rep(aspl_pint_h_2_1_pc$ci_upper, length(aspl_pint_h_2_1_pc$aspl_values)),
               rep(aspl_educ_l_pc$ci_upper, length(aspl_educ_l_pc$aspl_values)),
               rep(aspl_educ_h_pc$ci_upper, length(aspl_educ_h_pc$aspl_values)))
)

# Create the box plot and add CIs using geom_errorbar
aspl_plot <- ggplot(plot_data, aes(fill = group, y = aspl_values, x = type)) + 
  ...
  labs(title = "ASPL Distribution by Group with 95% CIs", y = "ASPL", fill = "Group", x = NULL) +  # Remove the x-axis label "type"
  coord_flip() +
...
res_educ <- resample(object = mnm_educ, data = mnm_educ_data, nB = 1000)
res_pint <- resample(object = mnm_pint, data = mnm_pint_data, nB = 1000)
...
r2_from_boots_with_ci <- function(df_boot) {
  ...
  # Calculate 95% bootstrapped confidence intervals using quantiles
  ci_lower <- quantile(r2_values_filtered, 0.025, na.rm = TRUE)  # 2.5th percentile
  ci_upper <- quantile(r2_values_filtered, 0.975, na.rm = TRUE)  # 97.5th percentile
...
# Prepare the data for box plot (using the filtered R² values without outliers)
plot_data <- data.frame(
...
# Create the box plot and add CIs using geom_errorbar
r2_plot <- ggplot(plot_data, aes(fill = group, y = r2_values, x = type)) + 
  geom_boxplot(position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, position = position_dodge(0.9)) + 
  scale_fill_manual(values = c("low" = "#966FD6B3", "high" = "#008B8BB3")) +
  labs(title = "R² Distribution by Group with 95% CIs", y = "R²", fill = "Group", x = NULL) +  # Remove the x-axis label "type"
  coord_flip() +
  scale_x_discrete(labels = c("pint" = "Political Interest", "educ" = "Education")) +  # Change axis labels
  theme_ipsum() +
  theme(
...
```{r}
# Save
Figure_3 <- (aspl_plot / facet_r2_plot / r2_plot) +
