...
p_load(tidyverse, here, sjlabelled, stringr, glue, EGAnet, janitor, haven,
       ...
       sjPlot, sna, grid, psych, stargazer, mgm, ggplot2, boot, stats,
...
generate_network_plots <- function(low_data, high_data, shortnames, longnames, group_names, group_colors, title_low, title_high, file_base) {
  ...
  low_data_net <- abs(low_data_net)  # Square the matrix
  ...
  high_data_net <- abs(high_data_net)  # Square the matrix
...
# mgm on att
  ...
  graph_mgm <- qgraph(mgm_att$pairwise$wadj, layout = 'spring',
                      labels = shortnames, nodeNames = longnames,  
                      groups = Totalgroup_comm, color = Totalgroup_cols, 
                      legend = F, legend.cex = 0.33, title = "mgm",
...
process_boots_data <- function(df_boot) {
  ...
  # Calculate the mean of non-zero values in the absolute value matrix for each matrix
...
#### Mann-Whitney U Test (constraint)

```{r}
# Create function to fit the test
run_mann_whitney <- function(group_low, group_high, label) {
  test_result <- wilcox.test(group_low, group_high, exact = FALSE)
...
# 1. Correlational networks
mw_pint_2_1 <- run_mann_whitney(cons_pint_l_2_1, cons_pint_h_2_1, "Cor: Political Interest (2_1)")
mw_pint_2_2 <- run_mann_whitney(cons_pint_l_2_2, cons_pint_h_2_2, "Cor: Political Interest (2_2)")
mw_educ <- run_mann_whitney(cons_educ_l, cons_educ_h, "Cor: Education")

# 2. Partial correlation networks 
mw_pint_2_1_pc <- run_mann_whitney(cons_pint_l_2_1_pc, cons_pint_h_2_1_pc, "PCor: Political Interest (2_1)")
mw_pint_2_2_pc <- run_mann_whitney(cons_pint_l_2_2_pc, cons_pint_h_2_2_pc, "PCor: Political Interest (2_2)")
mw_educ_pc <- run_mann_whitney(cons_educ_l_pc, cons_educ_h_pc, "PCor: Education")
```

```{r}
# Function for eff sizes (Cliff’s Delta)
compute_cliffs_delta <- function(group_low, group_high, label) {
  delta <- cliff.delta(group_low, group_high)
  
  # Print results
  cat("\n----------------------------------\n")
  cat("Cliff’s Delta for:", label, "\n")
  cat("----------------------------------\n")
  cat("Delta:", delta$estimate, "\n")
  cat("95% CI:", delta$conf.int[1], "to", delta$conf.int[2], "\n")
  cat("----------------------------------\n")
...
# 1. Correlational networks
cd_pint_2_1 <- compute_cliffs_delta(cons_pint_l_2_1, cons_pint_h_2_1, "Cor: Political Interest (2_1)")
cd_pint_2_2 <- compute_cliffs_delta(cons_pint_l_2_2, cons_pint_h_2_2, "Cor: Political Interest (2_2)")
cd_educ <- compute_cliffs_delta(cons_educ_l, cons_educ_h, "Cor: Education")

# 2. Partial correlation networks 
cd_pint_2_1_pc <- compute_cliffs_delta(cons_pint_l_2_1_pc, cons_pint_h_2_1_pc, "PCor: Political Interest (2_1)")
cd_pint_2_2_pc <- compute_cliffs_delta(cons_pint_l_2_2_pc, cons_pint_h_2_2_pc, "PCor: Political Interest (2_2)")
cd_educ_pc <- compute_cliffs_delta(cons_educ_l_pc, cons_educ_h_pc, "PCor: Education")
```

```{r}
# Violins for Mann results

# Define custom colors for low vs. high partitions
custom_colors <- c("Low" = "#966FD6B3", "High" = "#008B8BB3")

# Define function to create violins with tighter spacing
create_violin_plot <- function(data, subtitle) {
  data$Group <- factor(data$Group, levels = c("Low", "High"))  
  
  ggplot(data, aes(x = Group, y = MeanConstraint, fill = Group)) +
    geom_violin(trim = TRUE, scale = "count", width = 0.8, alpha = 0.8) +  
    geom_boxplot(width = 0.1, outlier.shape = NA, color = "black") +
    labs(subtitle = subtitle, x = "", y = "Mean Constraint") +
    ...
    theme(
      ...
      plot.margin = margin(10, 10, 10, 10),  # Ensure space for titles
...
df_pint_cor <- data.frame(
  MeanConstraint = c(cons_pint_l_2_2, cons_pint_h_2_2),
  Group = rep(c("Low", "High"), each = length(cons_pint_l_2_2))
)

df_pint_pc <- data.frame(
  MeanConstraint = c(cons_pint_l_2_2_pc, cons_pint_h_2_2_pc),
  Group = rep(c("Low", "High"), each = length(cons_pint_l_2_2_pc))
)

df_educ_cor <- data.frame(
  MeanConstraint = c(cons_educ_l, cons_educ_h),
  Group = rep(c("Low", "High"), each = length(cons_educ_l))
)

df_educ_pc <- data.frame(
  MeanConstraint = c(cons_educ_l_pc, cons_educ_h_pc),
  Group = rep(c("Low", "High"), each = length(cons_educ_l_pc))
)

# Create plots for Political Interest
p1 <- create_violin_plot(df_pint_cor, "Correlational network")
p2 <- create_violin_plot(df_pint_pc, "mgm")

# Create plots for Education
p3 <- create_violin_plot(df_educ_cor, "Correlational network")
p4 <- create_violin_plot(df_educ_pc, "mgm")

# Add Political Interest title
H1_title <- ggplot() + 
  annotate("text", x = 1, y = 1, label = "Political Interest", size = 10, fontface = "bold") +
  theme_void() +
  theme(plot.margin = margin(15, 0, 5, 0))  # Add extra top margin to prevent cropping

# Add Education title
H2_title <- ggplot() + 
  annotate("text", x = 1, y = 1, label = "Education", size = 10, fontface = "bold") +
  theme_void() +
  theme(plot.margin = margin(15, 0, 5, 0))  # Add extra top margin

# Arrange plots with titles, ensuring maximum space use
final_plot <- (
  H1_title / (p1 | p2) /  # Political Interest row
  H2_title / (p3 | p4)    # Education row
) + 
  plot_layout(heights = c(0.1, 1, 0.1, 1))  # Increase title row heights

# Save with optimized size
ggsave(here("Output", "Article", "CONSTRAINT_MANN.jpg"), 
       plot = final_plot, width = 12, height = 12, dpi = 300, bg = "white")
```


#### Violins (bootstrapped CIs)
...
H1_2_1 = create_plot(cons_pint_l_2_1, NULL, cons_pint_h_2_1, cons_pint_l_2_1_pc, NULL, cons_pint_h_2_1_pc, "Political Interest - Option 2") +
  ggtitle("Political Interest - Option 2") + 
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5))

# Create and print the H1_pint_2_2 plot (with two levels)
H1_2_2 = create_plot(cons_pint_l_2_2, NULL, cons_pint_h_2_2, cons_pint_l_2_2_pc, NULL, cons_pint_h_2_2_pc, "Political Interest") +
  ggtitle("Political Interest") + 
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5))
...
```{r}
# Function to perform Mann-Whitney U test node-wise
run_mann_whitney <- function(group_1, group_2, node_name, label) {
  test_result <- wilcox.test(group_1, group_2, exact = FALSE)
  
  # Print results
  cat("\n----------------------------------\n")
  cat("Mann-Whitney U Test for:", label, "-", node_name, "\n")
  cat("----------------------------------\n")
  cat("W statistic:", test_result$statistic, "\n")
  cat("p-value:", test_result$p.value, "\n")
  cat("----------------------------------\n")
...
mw_results_pint <- list()
mw_results_educ <- list()

# Perform Mann-Whitney U tests node-wise
for (node in names(r2_mat)) {
  
  # Extract data for this node (1000 rows × 4 columns)
  node_data <- r2_mat[[node]]
  
  # Compare Political Interest (polint_2_2_l vs. polint_2_2_h)
  mw_results_pint[[node]] <- run_mann_whitney(node_data[, 1], node_data[, 2], node, "Political Interest")
  
  # Compare Education (educ_low vs. educ_high)
  mw_results_educ[[node]] <- run_mann_whitney(node_data[, 3], node_data[, 4], node, "Education")
}

```

```{r}
# Initialize lists to store Cliff’s Delta results
cliffs_delta_pint <- list()
cliffs_delta_educ <- list()

# Compute Cliff’s Delta for each node
for (node in names(r2_mat)) {
  ...
  # Compare Political Interest (polint_2_2_l vs. polint_2_2_h)
  cliffs_delta_pint[[node]] <- cliff.delta(node_data[, 1], node_data[, 2])
  
  # Compare Education (educ_low vs. educ_high)
  cliffs_delta_educ[[node]] <- cliff.delta(node_data[, 3], node_data[, 4])
}

# Print Cliff’s Delta results for Political Interest
cat("\nCliff’s Delta for Political Interest\n")
print(cliffs_delta_pint)

# Print Cliff’s Delta results for Education
cat("\nCliff’s Delta for Education\n")
print(cliffs_delta_educ)
```

```{r}
# Define custom colors for Low vs. High groups
custom_colors <- c("Low" = "#966FD6B3", "High" = "#008B8BB3")  # Corrected color mapping

# Prepare data for plotting
plot_data_pint <- do.call(rbind, lapply(names(r2_mat), function(node) {
  data.frame(
    Node = node,
    R2 = c(r2_mat[[node]][, 1], r2_mat[[node]][, 2]),  # First col = polint_2_2_l, Second col = polint_2_2_h
    Group = rep(c("Low", "High"), each = nrow(r2_mat[[node]]))  # Label groups
  )
}))

# Ensure "Low" is always plotted first (before "High")
plot_data_pint$Group <- factor(plot_data_pint$Group, levels = c("Low", "High"))

# Create violin plot with facet wrapping
violin_plot_pint <- ggplot(plot_data_pint, aes(x = Group, y = R2, fill = Group)) +
  geom_violin(trim = TRUE, scale = "count", width = 0.8, alpha = 0.8) +
  geom_boxplot(width = 0.1, outlier.shape = NA, color = "black") +
  facet_wrap(~ Node, scales = "free_y", ncol = 5) +  # Arrange nodes in facets
  labs(title = "Political Interest - R² Comparison", x = "", y = "R² Values") +
  ...
  theme(
    ...
    strip.text = element_text(size = 12, face = "bold"),  # Node names in facets
    panel.spacing = unit(0.5, "lines"),
    legend.position = "none"
  )

# Save plot
ggsave(here("Output", "Article", "R2_Political_Interest_Violins.jpg"), 
       plot = violin_plot_pint, width = 12, height = 12, dpi = 300, bg = "white")

# Display the plot
print(violin_plot_pint)
```

```{r}
# Prepare data for plotting (Education)
plot_data_educ <- do.call(rbind, lapply(names(r2_mat), function(node) {
  data.frame(
    Node = node,
    R2 = c(r2_mat[[node]][, 3], r2_mat[[node]][, 4]),  # Third col = educ_low, Fourth col = educ_high
    Group = rep(c("Low", "High"), each = nrow(r2_mat[[node]]))  # Label groups
  )
}))

# Ensure "Low" is always plotted first (before "High")
plot_data_educ$Group <- factor(plot_data_educ$Group, levels = c("Low", "High"))

# Create violin plot with facet wrapping
violin_plot_educ <- ggplot(plot_data_educ, aes(x = Group, y = R2, fill = Group)) +
  geom_violin(trim = TRUE, scale = "count", width = 0.8, alpha = 0.8) +
  geom_boxplot(width = 0.1, outlier.shape = NA, color = "black") +
  facet_wrap(~ Node, scales = "free_y", ncol = 5) +  # Arrange nodes in facets
  labs(title = "Education - R² Comparison", x = "", y = "R² Values") +
  scale_fill_manual(values = custom_colors) +  # Corrected color mapping
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
# Save plot
ggsave(here("Output", "Article", "R2_Education_Violins.jpg"), 
       plot = violin_plot_educ, width = 12, height = 12, dpi = 300, bg = "white")
```
...
#Load
load(file = here("Input", "ASPL", "ASPL_all.RData"))
...
mann_whitney_results <- list(
  "Political Interest" = wilcox.test(aspl_results$aspl_pint_l, aspl_results$aspl_pint_h, alternative = "two.sided"),
  "Education" = wilcox.test(aspl_results$aspl_educ_l, aspl_results$aspl_educ_h, alternative = "two.sided")
)

# Print results
for (name in names(mann_whitney_results)) {
  cat("\n----------------------------------\n")
  cat("Mann-Whitney U Test for:", name, "\n")
  print(mann_whitney_results[[name]])
  cat("----------------------------------\n")
}
```
...
cliffs_delta_results <- list(
  "Political Interest" = cliff.delta(aspl_results$aspl_pint_l, aspl_results$aspl_pint_h),
  "Education" = cliff.delta(aspl_results$aspl_educ_l, aspl_results$aspl_educ_h)
)

# Print results
for (name in names(cliffs_delta_results)) {
  cat("\n----------------------------------\n")
  cat("Cliff’s Delta for:", name, "\n")
  print(cliffs_delta_results[[name]])
  cat("----------------------------------\n")
}
```
```{r}
...
plot_data <- data.frame(
  Type = rep(c("Political Interest", "Political Interest", "Education", "Education"), 
             times = c(length(aspl_results$aspl_pint_l), length(aspl_results$aspl_pint_h), length(aspl_results$aspl_educ_l), length(aspl_results$aspl_educ_h))),
  Group = rep(c("Low", "High", "Low", "High"), 
              times = c(length(aspl_results$aspl_pint_l), length(aspl_results$aspl_pint_h), length(aspl_results$aspl_educ_l), length(aspl_results$aspl_educ_h))),
  ASPL = c(aspl_results$aspl_pint_l, aspl_results$aspl_pint_h, aspl_results$aspl_educ_l, aspl_results$aspl_educ_h)
)

# Define custom colors for Low vs High partitions
custom_colors <- c("Low" = "#966FD6B3", "High" = "#008B8BB3")

# Create function to generate violin plots
create_violin_plot <- function(data, title) {
  ggplot(data, aes(x = Group, y = ASPL, fill = Group)) +
    geom_violin(trim = TRUE, scale = "count", width = 0.8, alpha = 0.8) +
    geom_boxplot(width = 0.1, outlier.shape = NA, color = "black") +
    labs(title = title, x = "", y = "ASPL") +
    scale_fill_manual(values = custom_colors) +
    theme_minimal(base_size = 14) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
...
p1 <- create_violin_plot(subset(plot_data, Type == "Political Interest"), "Political Interest")
p2 <- create_violin_plot(subset(plot_data, Type == "Education"), "Education")

# Arrange plots vertically
final_plot <- p1 / p2 + plot_layout(heights = c(1, 1))

# Save final plot
ggsave(here("Output", "Article", "ASPL_VIOLIN.jpg"), final_plot, width = 12, height = 8, dpi = 300)
```
...
#aspl_pint_l_2_2_pc <- aspl_from_boots_with_ci(df.boot_pint_l_2_2_pc)
#aspl_pint_h_2_2_pc <- aspl_from_boots_with_ci(df.boot_pint_h_2_2_pc)
#
...
# Save
save(aspl_pint_l_2_2_pc, file = here("Input", "ASPL", "aspl_pint_l.RData"))
save(aspl_pint_h_2_2_pc, file = here("Input", "ASPL", "aspl_pint_h.RData"))
...
plot_data <- data.frame(
  type = c(rep("pint", length(aspl_pint_l_2_2_pc$aspl_values)), rep("pint", length(aspl_pint_h_2_2_pc$aspl_values)),
           rep("educ", length(aspl_educ_l_pc$aspl_values)), rep("educ", length(aspl_educ_h_pc$aspl_values))),
  group = c(rep("low", length(aspl_pint_l_2_2_pc$aspl_values)), rep("high", length(aspl_pint_h_2_2_pc$aspl_values)),
            rep("low", length(aspl_educ_l_pc$aspl_values)), rep("high", length(aspl_educ_h_pc$aspl_values))),
  aspl_values = c(aspl_pint_l_2_2_pc$aspl_values, aspl_pint_h_2_2_pc$aspl_values, 
...
jpeg(here("Output", "Article", "Figure_4.jpg"), 
...
```{r}
#Figure 1 (2 in the article)

## cor net

# Remove diagonal elements (self-loops) if necessary
att_net_no_diag = att_net
diag(att_net_no_diag) <- NA

## Min edge weight (excluding diagonal elements)
min_fig1 = att_net_no_diag %>% 
  as.vector() %>%
  min(na.rm = TRUE)

## Max edge weight (excluding diagonal elements and edge weight of 1)
max_fig1 = att_net_no_diag %>% 
  as.vector()
max_fig1 = max_fig1[!is.na(max_fig1) & max_fig1 != 1]
max_fig1 = max(max_fig1, na.rm = TRUE)

## Node strength (using the original matrix if diagonals are meaningful)
strength_fig1 = centrality(att_net)
min_st_fig1 = min(strength_fig1$OutDegree)
max_st_fig1 = max(strength_fig1$OutDegree)

## Constraint (using the full matrix)
const_fig1 = mean(abs(att_net), na.rm = TRUE)

## Number of non-zero edges (ignoring diagonal elements if necessary)
nonzerofig1 = sum(att_net_no_diag != 0, na.rm = TRUE)

# Count edges lower than 0.001
entries_below_001 = sum(att_net_no_diag < 0.001, na.rm = TRUE)
```

```{r}
# Figure 1 right panel (2 in the artilce)

## mgm

#signed matrix
mat_mgm_att = mgm_att$pairwise$wadj*mgm_att$pairwise$signs

# Remove diagonal elements (self-loops) if necessary
mat_mgm_att_no_diag = mat_mgm_att
diag(mat_mgm_att_no_diag) <- NA

## Min edge weight (excluding diagonal elements)
min_fig1 = mat_mgm_att_no_diag %>% 
  as.vector() %>%
  min(na.rm = TRUE)

## Max edge weight (excluding diagonal elements and edge weight of 1)
max_fig1 = mat_mgm_att_no_diag %>% 
  as.vector()
max_fig1 = max_fig1[!is.na(max_fig1) & max_fig1 != 1]
max_fig1 = max(max_fig1, na.rm = TRUE)

## Node strength (using the original matrix if diagonals are meaningful)
strength_fig1 = centrality(mgm_att$pairwise$wadj)

## Constraint (using the full matrix)
const_fig1 = mean(abs(mgm_att$pairwise$wadj[!is.na(mgm_att$pairwise$wadj) & mgm_att$pairwise$wadj != 0]))

## Number of non-zero edges (ignoring diagonal elements if necessary)
zerofig1 = (sum(is.na(mgm_att$pairwise$signs)) - 19) / 2

# Count edges lower than 0.001
entries_no0 = sum(mgm_att$pairwise$wadj != 0, na.rm = TRUE)
```
...
# Function to calculate and print the mean and bootstrapped CIs
print_summary_stats <- function(df_l, df_m = NULL, df_h, df_l_pc, df_m_pc = NULL, df_h_pc) {
  if (!is.null(df_m)) {
    pint <- c(rep("Low", 10000), rep("Medium", 10000), rep("High", 10000))
    const_val <- c(df_l, df_m, df_h, df_l_pc, df_m_pc, df_h_pc)
    data_type <- rep(c("Correlational network", "mgm"), each = 30000)
    pint_vector <- rep(pint, 2)
  } else {
    pint <- c(rep("Low", 10000), rep("High", 10000))
    const_val <- c(df_l, df_h, df_l_pc, df_h_pc)
    data_type <- rep(c("Correlational network", "mgm"), each = 20000)
  ...
  mcon_data_summary <- mcon_data %>%
    group_by(pint, data_type) %>%
    summarise(
      mean_val = mean(const_val),
      ci_lower = quantile(const_val, 0.025),
      ci_upper = quantile(const_val, 0.975)
    )
  
  # Print the summary
  print(mcon_data_summary)
}

# print desc
print_summary_stats(cons_pint_l, cons_pint_m, cons_pint_h, cons_pint_l_pc, cons_pint_m_pc, cons_pint_h_pc)

# for two levels df:
print_descriptives <- function(cons_pint_h_2_2, cons_pint_h_2_2_pc, cons_pint_l_2_2, cons_pint_l_2_2_pc) {
  # Combine the data
  cons_data <- data.frame(
    const_val = c(cons_pint_h_2_2, cons_pint_h_2_2_pc, cons_pint_l_2_2, cons_pint_l_2_2_pc),
    group = rep(c("cons_pint_h_2_2", "cons_pint_h_2_2_pc", "cons_pint_l_2_2", "cons_pint_l_2_2_pc"), 
                each = length(cons_pint_h_2_2))
  )
  
  # Calculate summary statistics
  cons_data_summary <- cons_data %>%
    group_by(group) %>%
    summarise(
      mean_val = mean(const_val),
      sd_val = sd(const_val),
      min_val = min(const_val),
      max_val = max(const_val),
      ci_lower = quantile(const_val, 0.025),
      ci_upper = quantile(const_val, 0.975)
    )
  
  # Print the summary
  print(cons_data_summary)
}

# Example usage (assuming cons_pint_h_2_2, cons_pint_h_2_2_pc, cons_pint_l_2_2, and cons_pint_l_2_2_pc are defined)
print_descriptives(cons_pint_h_2_2, cons_pint_h_2_2_pc, cons_pint_l_2_2, cons_pint_l_2_2_pc)

```

```{r}
# Figure 3 (4 in the article): ASPL
# Function to print descriptive statistics for ASPL values
print_aspl_descriptives <- function(aspl_data) {
  aspl_values <- aspl_data$aspl_values
  
  # Calculate the mean, standard deviation, quartiles, and bootstrapped confidence intervals
  mean_val <- mean(aspl_values, na.rm = TRUE)
  sd_val <- sd(aspl_values, na.rm = TRUE)
  quartiles <- quantile(aspl_values, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
  ci_lower <- quantile(aspl_values, 0.025, na.rm = TRUE)
  ci_upper <- quantile(aspl_values, 0.975, na.rm = TRUE)
  
  # Print the results
  cat("Descriptive Statistics for ASPL:\n")
  cat("Mean:", mean_val, "\n")
  cat("Standard Deviation:", sd_val, "\n")
  cat("25th Percentile (Q1):", quartiles[1], "\n")
  cat("50th Percentile (Median):", quartiles[2], "\n")
  cat("75th Percentile (Q3):", quartiles[3], "\n")
  cat("Lower 95% CI:", ci_lower, "\n")
  cat("Upper 95% CI:", ci_upper, "\n")
}

# print desc
print_aspl_descriptives(aspl_pint_l_2_2_pc)
print_aspl_descriptives(aspl_pint_h_2_2_pc)
print_aspl_descriptives(aspl_educ_l_pc)
print_aspl_descriptives(aspl_educ_h_pc)

```

```{r}
# Figure 3 (4 in the article): R2 sums
# Function to print descriptive statistics for R² values
print_r2_descriptives <- function(r2_data) {
  r2_values <- r2_data$r2_values
  
  # Calculate the mean, standard deviation, quartiles, and bootstrapped confidence intervals
  mean_val <- mean(r2_values, na.rm = TRUE)
  sd_val <- sd(r2_values, na.rm = TRUE)
  quartiles <- quantile(r2_values, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
  ci_lower <- quantile(r2_values, 0.025, na.rm = TRUE)
  ci_upper <- quantile(r2_values, 0.975, na.rm = TRUE)
  
  # Print the results
  cat("Descriptive Statistics for R²:\n")
  cat("Mean:", mean_val, "\n")
  cat("Standard Deviation:", sd_val, "\n")
  cat("25th Percentile (Q1):", quartiles[1], "\n")
  cat("50th Percentile (Median):", quartiles[2], "\n")
...
# Example usage with your R² objects
print_r2_descriptives(r2_pint_l_2_1_pc)
print_r2_descriptives(r2_pint_h_2_1_pc)
print_r2_descriptives(r2_educ_l_pc)
print_r2_descriptives(r2_educ_h_pc)
```
