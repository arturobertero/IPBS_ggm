...
ggsave(here("Output", "Article", "Figure_3.png"), Figure_3, width = 12, height = 16, dpi = 300)
```
...
tab_mod_df_educ = tab_mod_educ %>%
  select(Variable.A, Variable.B, Mean, qtl_low, qtl_high, Mod_Mean, Mod_qtl_low, Mod_qtl_high, Mod_propLtZ) %>% 
  filter(Mod_propLtZ > 0.00) %>%  # Filter for moderation effect detected in more than 80% of bootstrap samples
...
# Update: Include columns for pairwise interactions Mean, qtl_low, and qtl_high
tab_mod_df_vote <- tab_mod_vote %>%
  select(Variable.A, Variable.B, Mean, qtl_low, qtl_high, Mod_Mean, Mod_qtl_low, Mod_qtl_high, Mod_propLtZ) %>% 
  filter(Mod_propLtZ > 0.00) %>%  # Filter for moderation effect detected in more than 40% of bootstrap samples
  arrange(desc(Mod_Mean))
...
# Add predicted0 and predicted1 columns to each dataframe
add_predicted_columns <- function(df) {
  df <- df %>%
    mutate(predicted0 = Mean + (Mod_Mean * 0),  # Prediction for moderator = 0
           predicted1 = Mean + (Mod_Mean * 1))  # Prediction for moderator = 1
  return(df)
}

# Apply the function to each dataframe
tab_mod_df_vote <- add_predicted_columns(tab_mod_df_vote)
tab_mod_df_educ <- add_predicted_columns(tab_mod_df_educ)
tab_mod_df_pint <- add_predicted_columns(tab_mod_df_pint)
...
# Step 2: Plot the graph with the updated Node labels on the y-axis
plot_mod_vote = ggplot(signed_mod_vote) +
  ...
  xlab("") +
  ylab("Signed Weight") +
  ggtitle("Predicted Signed Weights for Interactions")
```
...
```{r}
# Plot for signed_mod_educ
plot_mod_educ = ggplot(signed_mod_educ) +
  # Plot predicted0 with color #966FD6B3
  geom_point(aes(x = Node, y = predicted0, color = "Predicted 0"), size = 3) +
...
```{r}
# Step 2: Plot the graph with the updated Node labels on the y-axis
plot_mod_pint = ggplot(signed_mod_pint) +
  # Plot predicted0 with color #966FD6B3
  geom_point(aes(x = Node, y = predicted0, color = "Predicted 0"), size = 3) +
