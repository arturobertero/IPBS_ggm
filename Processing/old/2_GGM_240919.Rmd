...
mnm_educ = mgm(mnm_educ_data, type = c(rep("g", 19), "c"), level = c(rep(1, 19), 2),
               moderators = 20, lambdaSel = "EBIC", ruleReg = "OR")
...
# You now have the tab_mod_df_educ and tab_mod_df_pint dataframes
# These dataframes have a similar structure to tab_mod_df but are built from the resampling results
...
# Conditioning the model on each level of vote_cat
cond1 <- condition(object = mnm_vote, values = list('20' = 1))  # Sin
cond2 <- condition(object = mnm_vote, values = list('20' = 2))  # M5S
cond3 <- condition(object = mnm_vote, values = list('20' = 3))  # Dx
...
# Multiplot
jpeg(here("Output", "Article", "mnm_vote_adjusted.jpg"), 
     height = 6000, width = 6000, quality = 1000)  # Adjust image size as needed

# Create a common layout based on all three conditions
L <- averageLayout(Graph_cond_1, Graph_cond_2, Graph_cond_3, layout = "spring")

# Matrix layout for the graphs (1 row, 3 columns)
lmat <- matrix(1:3, 1, 3)

# Start plotting
layout(lmat, width = c(1, 1, 1))

# Generate plots with consistent layout and edge fading based on weights
set.seed(1)
Graph_cond_1 <- qgraph(
  ...
  maximum = max(c(cond1$pairwise$wadj, cond2$pairwise$wadj, cond3$pairwise$wadj))  # Consistent scaling across all plots
)
